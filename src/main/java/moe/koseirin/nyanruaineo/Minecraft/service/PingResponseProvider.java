...
import moe.koseirin.nyanruaineo.Minecraft.config.cfg.MotdEntry;
import moe.koseirin.nyanruaineo.Minecraft.protocol.ProtocolVersion;
import moe.koseirin.nyanruaineo.Minecraft.util.FileUtils;
import moe.koseirin.nyanruaineo.Minecraft.util.PlaceholderResolver;
import moe.koseirin.nyanruaineo.Minecraft.util.RGBColorConverter;
...
public class PingResponseProvider {
    ...
    public void reloadIcon() {
        try {
            ...
            if (iconFile.exists() && iconFile.isFile()) {
                ...
                // The favicon field must carry a real 64x64 PNG (the client only decodes PNG); a
                // file with another format (e.g. a mislabelled JPEG) would silently break the icon.
                if (!isPng(bytes)) {
                    log.error("Server icon at {} is not a PNG image ({} bytes); it will not display. "
                            + "Please replace it with a 64x64 PNG.", ICON_PATH, bytes.length);
                    faviconBase64.set("");
                    return;
                }
                String base64 = Base64.getEncoder().encodeToString(bytes);
                faviconBase64.set(FAVICON_PREFIX + base64);
                log.info("Loaded server icon from {} ({} bytes)", ICON_PATH, bytes.length);
            } else {
                faviconBase64.set("");
    ...
    /** Whether the bytes carry the PNG signature. */
    ...
    public String getPingJson(int clientProtocol, int realOnline) {
        ...
        // 获取 MOTD 配置
        MotdConfig motdConfig = properties.getMotdConfig();
        ...
        if (motdConfig.getRandomMotds() != null && !motdConfig.getRandomMotds().isEmpty()) {
            int totalWeight = motdConfig.getRandomMotds().stream()
                    .mapToInt(MotdEntry::getWeight).sum();
            int r = random.nextInt(totalWeight);
            int cumulative = 0;
            MotdEntry selected = null;
            for (MotdEntry entry : motdConfig.getRandomMotds()) {
